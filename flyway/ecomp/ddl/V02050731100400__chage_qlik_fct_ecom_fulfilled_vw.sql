...
SELECT
  ...
  NVL(CAST(${ecomp_catalog}.ecom.custom_concat_ifnull(ARRAY(CAST(a.chain_key AS INT), '0', ABS(a.dks_sku))) AS DOUBLE), -1.0) AS ECOM_WEB_SKU_KEY,
  ...
  SUBSTR(b.cust_service_rep_code,1,NVL(NULLIF(INSTR(b.cust_service_rep_code, '@') - 1, -1), 20))  AS ECOM_TX6_CS_REP,
...
FROM ${ecomp_catalog}.ecom_dim.txn_order_sku a
LEFT JOIN ${ecomp_catalog}.ecom_dim.cust_service_rep b
  ON a.txn_cust_service_rep_key = b.cust_service_rep_key
LEFT JOIN ${ecomp_catalog}.ecom_dim.order_fulfill c
  ON a.order_fulfill_key = c.order_fulfill_key
LEFT JOIN ${ecomp_catalog}.ecom_dim.order_delivery d
  ON a.order_delivery_key = d.order_delivery_key
WHERE
  a.trans_type_key = 6
  AND a.txn_date_key BETWEEN 20230129
  AND CAST(DATE_FORMAT(DATE_SUB(current_date(), 1), 'yyyyMMdd') AS BIGINT)
