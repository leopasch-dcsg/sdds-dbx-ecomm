...
from ecmde_ecomm.fulfillment.notebooks.fulfillment_operations import (
...
try:
    ...
    oso_catalog = NotebookUtil.notebook_param("oso_source_catalog")
    fit_catalog = NotebookUtil.notebook_param("fit_source_catalog")
    ecomp_catalog = NotebookUtil.notebook_param("ecomp_catalog")

    provider = OrdersMergeOperationProvider.provider(
        oso_catalog,
        fit_catalog,
        ecomp_catalog,
        config,
    ...
    result = provider.operation(data_quality).execute()
