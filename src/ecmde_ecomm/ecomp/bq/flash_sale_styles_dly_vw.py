...
class FlashSaleStylesDlyEgressOperation(BigQueryEgressOperation):
    ...
    def get_source_dataframe(
        ...
        return df.select(
            ...
            date_format(
                convert_timezone(lit("UTC"), lit(timezone), col("START_DATE")),
                timestamp_format,
            )
            .cast(StringType())
            .alias("start_date"),
            date_format(
                convert_timezone(lit("UTC"), lit(timezone), col("END_DATE")),
                timestamp_format,
            )
            .cast(StringType())
            .alias("end_date"),
